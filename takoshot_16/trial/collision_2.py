...
class Scene_TryCollision2(Scene):
    ...
    def prepare(self):
        ...
        self.user_player = self.game.world.add_player(140, 140)
    ...
    def update(self):
        ...
        if (input_sys.is_push(KeyCodes.MOUSE_RIGHT)):
            self.game.world.collision.break_block_in_circle(*input_sys.mouse_pos, self.RADIUS)
        # プレイヤーにダメージを与える
        if (input_sys.is_push(KeyCodes.Z)):
            for col_info in self.game.world.collision.intersect_circle_and_players(*input_sys.mouse_pos, self.RADIUS, self.game.world.player_list):
