...
class Transform:
    ...
    # ビュー領域がワールド内に収まるためのズームと位置を計算する
    #
    # bbox ビュー領域が含まれるx, y軸に平行な長方形
    @classmethod
    def calc_viewarea_in_world2(self, bbox, base_viewarea_size, range_of_viewarea):
    
        bbox = BBox(bbox.left, bbox.top, bbox.right, bbox.bottom) # copy
        base_width, base_height = base_viewarea_size
        out_of_viewarea = range_of_viewarea.inflate(2, 2)
        rate_x = out_of_viewarea.width / bbox.width
        rate_y = out_of_viewarea.height / bbox.height
        rate_min = min(rate_x, rate_y)
        
        out_of_world = False
        scale = bbox.width / base_width
        
        # ビュー領域がout_of_viewareaを超えたら
        # out_of_viewareaにおさまるようにする
        if (rate_min < 1.0):
            ...
            # x方向がはみ出た
            if (rate_x < rate_y):
                # left, rightはrange_of_viewareaの境界で決定
                bbox.left,  bbox.right = range_of_viewarea.left, range_of_viewarea.right
                scale = (bbox.right - bbox.left) / base_width # カメラの倍率を計算
                # 新しいカメラ倍率でtop, bottom, heightを再計算
                bbox.top = bbox.centery - base_height * scale * 0.5
                bbox.bottom = bbox.centery + base_height * scale * 0.5
                bbox.height = bbox.bottom - bbox.top
            else: # y方向がはみ出た
                # top, bottomはrange_of_viewareaの境界で決定
                bbox.top,  bbox.bottom = range_of_viewarea.top, range_of_viewarea.bottom
                scale = (bbox.bottom - bbox.top) / base_height # カメラの倍率を計算
                # 新しいカメラ倍率でleft, right, widthを再計算
                bbox.left = bbox.centerx - base_width * scale * 0.5
                bbox.right = bbox.centerx + base_width * scale * 0.5
                bbox.width = bbox.right - bbox.left
        
        # ビュー領域が範囲外にいる場合、範囲内に戻す
        if (bbox.left < out_of_viewarea.left):
            out_of_world = True
            bbox.left = range_of_viewarea.left
            bbox.right = range_of_viewarea.left + bbox.width
        
        if (out_of_viewarea.right < bbox.right):
            out_of_world = True
            bbox.right = range_of_viewarea.right
            bbox.left = range_of_viewarea.right - bbox.width
        
        if (bbox.top < out_of_viewarea.top):
            out_of_world = True
            bbox.top = range_of_viewarea.top
            bbox.bottom = range_of_viewarea.top + bbox.height
        
        if (out_of_viewarea.bottom < bbox.bottom):
            out_of_world = True
            bbox.bottom = range_of_viewarea.bottom
            bbox.top = range_of_viewarea.bottom - bbox.height
        
        center_x = (bbox.left + bbox.right) * 0.5
        center_y = (bbox.top + bbox.bottom) * 0.5
        
        # check
        err = False
        if (g.DebugFlags.CHECK_calc_viewarea_in_world2):
            diff_list = [v for v in [out_of_viewarea.top - bbox.top, out_of_viewarea.left - bbox.left, bbox.right - out_of_viewarea.right, bbox.bottom - out_of_viewarea.bottom ]]
            err = True in [0 <= v for v in diff_list]
            
            if (g.DebugFlags.PRINT_calc_viewarea_in_world2):
                print(err, diff_list)
        
        return out_of_world, center_x, center_y, scale, (err, bbox)
