...
class Scene_MainGame(Scene_Container):
    ...
    def __init__(self, btlinit_info):
        super().__init__(g.systems, SceneId.MAINGAME)
    ...
    def on_render(self):
        ...
        # 風
        gra_ex.draw_windy(pygame.Rect(20, 500, 80, 16), self.game.world.windy)
        ...
        self.ui_player.ex_prop.setdefault('ui_power_rate', -1)
        gra_ex.draw_shotbar(self.SHOTBAR_RECT, self.shotpower_rate, self.tri0_pos, self.tri1_pos, self.drag_event_obj, self.ui_player.ex_prop['ui_power_rate']) #self.tri0_pos, self.tri1_pos, -1)
    ...
    def on_end_child_scene(self, child):
        ...
        if (child.id == SceneId.MAINGAME_INTRO):
            ...
            self.try_next_or_result()
        elif (child.id == SceneId.MAINGAME_BEFORE):
            ...
            if child.has_result_ok(): # ショットあり
                ...
                self.add_child(Scene_TurnEnd(self))
    ...
    def try_next_or_result(self):
        if (not self.try_scene_game_result()):
            self.add_child(Scene_Before(self))
    ...
    # 落下中のプレイヤーがいる
    def check_falling_player(self):
        alive_players = [player for player in self.game.world.player_list if player.is_alive()]
        
        for player in alive_players:
            if (not player.is_land):
                return True
...
class Scene_Intro(Scene):
    def __init__(self, parent_scene):
        super().__init__(g.systems, SceneId.MAINGAME_INTRO)
...
class Scene_ReadyGo(Scene):
    def __init__(self):
        super().__init__(g.systems, SceneId.MAINGAME_READYGO)
...
# プレイヤーが行動する前
class Scene_Before(Scene):

    def __init__(self, parent_scene):
        super().__init__(g.systems, SceneId.MAINGAME_BEFORE)
    ...
    def prepare(self):
        ...
        # 一定のターンが経過
        if (world.windy.time_passes()):
        
            # 風をランダムに変更
            world.windy.set_random_value()
            
            # プレイヤーにイベント発生（アイテムの配布など）
            for player in world.player_list:
                if (player.is_alive()):
                    player.on_next_phase()
            
            play_se(g.systems, "se_next_phase")
            self.timer.start(duration_time_ms = 1250)
        else:
            self.timer.start(duration_time_ms = 0)
...
class Scene_ControlPlayer(Scene_Container):
    ...
    def __init__(self, parent_scene, player):
        super().__init__(g.systems, SceneId.MAINGAME_CONTROL_PLAYER)
    ...
    def control_shot(self):
        ...
        if (input_sys.is_keyup(KeyCodes.SPACE) or self.shooting_timer.is_finished()):
            ...
            self.player.ex_prop['ui_power_rate'] = self.shooting_timer.progress
            return True
        if (input_sys.is_keydown(KeyCodes.W)):
            cmd.decrease_shooting_angle()
        if (input_sys.is_keydown(KeyCodes.S)):
...
class Scene_WaitLandingShell(Scene):
    ...
    # player  弾を撃ったプレイヤー
    def __init__(self, parent_scene, player):
        super().__init__(g.systems, SceneId.MAINGAME_WAIT_LANDING_SHELL)
...
class Scene_TurnEnd(Scene):
    def __init__(self, parent_scene):
        super().__init__(g.systems, SceneId.MAINGAME_TURN_END)
    ...
    def update(self):
        # 落下中のプレイヤーがいる
        if (self.parent_scene.check_falling_player()): return
...
class Scene_GameResult(Scene):
    ...
    def __init__(self, winner_players):
        super().__init__(g.systems, SceneId.MAINGAME_RESULT)
    ...
    def update(self):
        if (self.timer.is_finished() and
            g.systems.input_sys.is_push(KeyCodes.MOUSE_LEFT) and g.systems.mouse_target_sys.current_target_id == MouseTargetId.RESULT_OK):
            self.end_scene()
    ...
    def render(self):
        ...
        if (self.timer.is_finished()):
            gra_ex.draw_button(rect.centerx - 40, rect.bottom - 60, 80, 32, "OK", MouseTargetId.RESULT_OK)
...
class Scene_Pause(Scene):
    ...
    def __init__(self):
        super().__init__(g.systems, SceneId.MAINGAME_PAUSE)
...
if __name__ == '__main__':
    import main
    from defines import BattleInitInfo
    
    def scene_factory():
        return Scene_MainGame(BattleInitInfo.create_1vs1())
    
    main.main(scene_factory, window_width=1200)
