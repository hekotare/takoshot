...
import globals as g
from geometry import (Geometry, Matrix, create_bounding_box_by_points,
                      rect_to_points)
...
def blit(surface, img, pos_or_rect, **kwargs):
    ...
    pos_or_rect = __to_round(pos_or_rect)
...
def blit2(surface, img, pos, scale, angle_deg, origin_pos, **kwargs):
    scaling_img = pygame.transform.scale(img, (img.get_width() * scale, img.get_height() * scale))
    rotate_blit(surface, scaling_img, pos, (scaling_img.get_width() * origin_pos[0], scaling_img.get_height() * origin_pos[1]), angle_deg, **kwargs)

def blit3(surface, src_img, pos, scale, angle_deg, origin_pos, **kwargs):
    clip_rect = kwargs.get('clip_rect')
    
    if (clip_rect):
        img, origin_x, origin_y = clipping_img(src_img, pos, scale, angle_deg, origin_pos, clip_rect)
    else:
...
def clipping_img(img, pos, scale, angle_deg, origin_pos, clipping_rect):
    ...
    return cliped_img, origin_x, origin_y
...
class Graphics:
    ...
    # pos, scale, angle_deg     描画する座標とスケール、回転
    # origin_pos                画像の中心座標（x=0～1.0, y=0～1.0)
    def blit2(self, img, pos, scale, angle_deg, origin_pos, **kwargs):
        blit2(self.target, img, pos, scale, angle_deg, origin_pos, **kwargs)

    def blit3(self, img, pos, scale, angle_deg, origin_pos, **kwargs):
        blit3(self.target, img, pos, scale, angle_deg, origin_pos, **kwargs)